...
const ANALYZE_BUTTON_IDS = ["analyze1p","analyzeGG","analyze2plus","savePdf","prepareDay"];
...
function buildNarrative(m, marketHint) {
  ...
  const isO15 = marketHint === '1h_over15';

  let s = `${m.team1} vs ${m.team2}: ligaški baseline za 1H gol je oko ${leagueBase}. `;
  ...
  s += `Spajanje je urađeno po preciznosti (effN prior=${effPrior}, micro=${effMicro}; udeo micro≈${wshare}), što daje konačnih ${fmt(m.final_percent, '%')}.`;
  return s;
}
...
function renderResults(data, market) {
  ...
  const cardHTML = (m) => {
    ...
    const isO15 = currentMarket === '1h_over15';
        ...
        <div style="margin:6px 0;padding:8px;background:#fafafa;border-radius:8px">
          ...
          ${isGG ? `
            <div>Team 1 scores 1H: <strong>${fmt(d.p_home_scores_1h, '%')}</strong>,
                 ...
                 <span style="color:#666">${d.rho!=null?`(ρ=${fmt(d.rho)})`:''}</span>
            </div>` : ``}
            ${isO15 ? `
            <div>Poisson λ: total <strong>${fmt(d.lambda_total)}</strong>
                <span style="color:#666">(home ${fmt(d.lambda_home)}, away ${fmt(d.lambda_away)})</span>
            </div>` : ``}
          <div>Merged (precision-weighted): <span style="color:#333">micro share ≈ ${fmt(d.merge_weight_micro)}</span></div>
          <div style="color:#666">effN prior: ${fmt(d.effn_prior)}, effN micro: ${fmt(d.effn_micro)}, ligaški baseline: ${fmt(d.m_league, '%')}</div>
        </div>
