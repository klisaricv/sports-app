...
@app.get("/api/global-loader-events")
async def api_global_loader_events():
    """Server-Sent Events endpoint za real-time global loader updates"""
    ...
    async def event_generator():
        ...
        while True:
            try:
                ...
                # Pošalji update samo ako se status promenio
                if current_status != last_status:
                    yield f"data: {json.dumps(current_status)}\n\n"
                    last_status = current_status
                    
                    # Ako nema aktivnog job-a, završi SSE
                    if not current_status["active"]:
                        break
                
                # Čekaj 1 sekund pre sledeće provere
