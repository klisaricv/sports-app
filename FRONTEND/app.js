...
function initUsersPageFunctionality() {
  console.log("🔍 [DEBUG] Initializing users page functionality");
...
function setupUsersEventListeners() {
  ...
  if (clearSearch) {
    clearSearch.addEventListener('click', clearUsersSearch);
  }
  ...
  if (prevBtn) {
    prevBtn.addEventListener('click', () => changeUsersPage(currentPage - 1));
  }
  
  if (nextBtn) {
    nextBtn.addEventListener('click', () => changeUsersPage(currentPage + 1));
  }
}
...
function updateUsersStats() {
  ...
  if (showingUsers) showingUsers.textContent = filteredUsers.length;
}
...
function renderUsersPagination() {
  ...
  if (paginationPages) {
    ...
    for (let i = startPage; i <= endPage; i++) {
      ...
      pageBtn.addEventListener('click', () => changeUsersPage(i));
